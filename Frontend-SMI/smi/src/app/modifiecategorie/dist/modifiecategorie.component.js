"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
exports.__esModule = true;
exports.ModifiecategorieComponent = void 0;
var core_1 = require("@angular/core");
var dialog_1 = require("@angular/material/dialog");
var sweetalert2_1 = require("sweetalert2");
var ModifiecategorieComponent = /** @class */ (function () {
    function ModifiecategorieComponent(dialogRef, userService, data) {
        this.dialogRef = dialogRef;
        this.userService = userService;
        this.data = data;
    }
    ModifiecategorieComponent.prototype.ngOnInit = function () {
        this.categoryId = this.data.categoryId;
        this.categoryDetails = this.data.categoryDetails;
    };
    ModifiecategorieComponent.prototype.updateCategory = function () {
        var _this = this;
        this.userService.updateCategory(this.categoryId, this.categoryDetails).subscribe(function (updatedCategory) {
            console.log('Catégorie mise à jour avec succès : ', updatedCategory);
            // Afficher une pop-up de succès
            sweetalert2_1["default"].fire({
                icon: 'success',
                title: 'Catégorie mise à jour avec succès!',
                showConfirmButton: false,
                timer: 1500
            });
            // Fermer le modal avec un résultat positif
            _this.dialogRef.close(true);
        }, function (error) {
            console.error('Erreur lors de la mise à jour de la catégorie : ', error);
            // Afficher une pop-up d'erreur
            sweetalert2_1["default"].fire({
                icon: 'error',
                title: 'Erreur lors de la mise à jour de la catégorie',
                text: 'Veuillez réessayer plus tard.'
            });
            // Fermer le modal avec un résultat négatif
            _this.dialogRef.close(false);
        });
    };
    ModifiecategorieComponent = __decorate([
        core_1.Component({
            selector: 'app-modifiecategorie',
            templateUrl: './modifiecategorie.component.html',
            styleUrls: ['./modifiecategorie.component.css']
        }),
        __param(2, core_1.Inject(dialog_1.MAT_DIALOG_DATA))
    ], ModifiecategorieComponent);
    return ModifiecategorieComponent;
}());
exports.ModifiecategorieComponent = ModifiecategorieComponent;
